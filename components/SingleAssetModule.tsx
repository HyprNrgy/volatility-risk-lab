...
export default function SingleAssetModule() {
  ...
  useEffect(() => {
    ...
    // Auto-compute on first load for better "first look" experience
    handleCompute();
  ...
  const handleCompute = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        // If missing parameters (like on first load before we have the date), don't throw a red error
        if (json.error === 'Missing required parameters') {
          return;
        }
      ...
      // Trigger evaluation for alerts engine
      if (json.metrics && json.metrics.annualizedVolatility) {
         alertsEngine.evaluateVolatility(ticker.toUpperCase(), json.metrics.annualizedVolatility * 100);
         
         const sentimentScore = json.metrics.meanReturn > 0.05 ? 85 : json.metrics.meanReturn < -0.05 ? 25 : 55;
         alertsEngine.evaluateSentiment(ticker.toUpperCase(), sentimentScore);
      }
    } catch (err: any) {
    ...
    <div className="space-y-6">
      ...
      {!data && !loading && !error && (
        <div className="flex flex-col items-center justify-center py-24 text-zinc-500 space-y-4 border border-zinc-800/50 rounded-xl bg-zinc-900/20 border-dashed">
          ...
          <div className="text-center space-y-1">
            <h3 className="text-zinc-400 font-medium">Ready to Analyze</h3>
      ...
      {loading && (
        <div className="space-y-6 animate-pulse">
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {[1, 2, 3, 4, 5, 6].map((i) => (
              <div key={i} className="bg-zinc-900 border border-zinc-800 p-4 rounded-xl h-24"></div>
            ))}
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-xl h-72"></div>
            <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-xl h-72"></div>
          </div>
        </div>
      )}
      ...
      {data && (
        <>
          ...
          <DecisionEngine 
            volatilityScore={Math.min(data.metrics.annualizedVolatility * 100, 100)} 
            sentimentScore={data.metrics.meanReturn > 0.05 ? 85 : data.metrics.meanReturn < -0.05 ? 25 : 55} 
            priceTrend={data.metrics.meanReturn > 0 ? 'up' : data.metrics.meanReturn < 0 ? 'down' : 'flat'} 
          />
              ...
              <div className="h-72">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={data.chartData}>
                    <CartesianGrid strokeDasharray="3 3" stroke="#3f3f46" vertical={false} />
                    <XAxis dataKey="date" stroke="#a1a1aa" fontSize={12} tickFormatter={(t) => t.substring(0, 7)} />
                    <YAxis stroke="#a1a1aa" fontSize={12} tickFormatter={(v) => (v * 100).toFixed(2) + '%'} />
                    <Tooltip
                      contentStyle={{ backgroundColor: '#18181b', borderColor: '#27272a', color: '#f4f4f5' }}
